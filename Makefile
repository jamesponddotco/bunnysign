...
doc: # Builds the manpage.
	$(SCDOC) <doc/bunnysign.1.scd >bunnysign.1

install: # Installs the release binary.
	$(INSTALL) -d \
		$(DESTDIR)$(PREFIX)/$(BINDIR)/ \
		$(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
	$(INSTALL) -pm 0755 bunnysign $(DESTDIR)$(PREFIX)/$(BINDIR)/
	$(INSTALL) -pm 0644 bunnysign.1 $(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
...
clean: # Cleans cache files from tests and deletes any build output.
	$(RM) -f cover.out bunnysign bunnysign.1
